...
export default function TagDetailPage() {
  ...
  const statusMessages: Record<TagStatus, { text: string; color: string }> = {
    VIRGIN: { text: 'Este tag aún no ha sido activado. Comparte el link con el cliente.', color: 'text-gray-500 dark:text-gray-400' },
    INCOMPLETE: { text: 'El cliente ha creado su PIN pero aún no completa sus datos.', color: 'text-amber-600 dark:text-amber-400' },
    ACTIVE: { text: 'Tag operativo. Los datos médicos están disponibles para paramédicos.', color: 'text-emerald-600 dark:text-emerald-400' },
    SUSPENDED: { text: 'Tag dado de baja.', color: 'text-red-600 dark:text-red-400' },
  };
  ...
  return (
    <DashboardLayout user={user}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        <div className="flex items-center gap-4 min-w-0">
          <button
            onClick={() => router.back()}
            aria-label="Volver"
            className="p-2 text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700/50 rounded-lg transition-colors shrink-0 focus-visible:ring-2 focus-visible:ring-red-500 focus:outline-none"
          >
            <ArrowLeft className="w-5 h-5" aria-hidden="true" />
          </button>
          <div className="min-w-0">
            <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Detalle del Tag</h1>
      ...
      <div className={`rounded-xl p-4 border mb-6 ${
        tag.status === 'VIRGIN' ? 'bg-gray-100 dark:bg-gray-800/30 border-gray-200 dark:border-gray-700/50' :
      ...
      <div className="grid lg:grid-cols-3 gap-6 mb-6">
        ...
        <div className="bg-white dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700/50 rounded-xl p-6">
          <h2 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-4">Información</h2>
        ...
        {/* Actions */}
        <div className="bg-white dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700/50 rounded-xl p-6">
          <h2 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-4">Acciones</h2>
          <div className="space-y-2">
            <button
              onClick={copyLink}
              className="w-full flex items-center gap-3 px-4 py-3 bg-blue-500/10 text-blue-600 dark:text-blue-400 font-medium rounded-lg hover:bg-blue-500/20 transition-colors"
            >
            ...
            <button
              onClick={openViewer}
              className="w-full flex items-center gap-3 px-4 py-3 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 font-medium rounded-lg hover:bg-emerald-500/20 transition-colors"
            >
            ...
            {tag.status !== 'VIRGIN' && tag.status !== 'SUSPENDED' && isAdmin && (
              <button
                ...
                className="w-full flex items-center gap-3 px-4 py-3 bg-amber-500/10 text-amber-600 dark:text-amber-400 font-medium rounded-lg hover:bg-amber-500/20 transition-colors"
              >
            ...
            {tag.status !== 'SUSPENDED' && isAdmin && (
              <button
                ...
                className="w-full flex items-center gap-3 px-4 py-3 bg-red-500/10 text-red-600 dark:text-red-400 font-medium rounded-lg hover:bg-red-500/20 transition-colors"
              >
        ...
        {isAdmin && tag.status === 'ACTIVE' && viewerData?.medicalData && (
          <div className="bg-white dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700/50 rounded-xl p-6">
            <h2 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-4">Datos Médicos</h2>
            <div className="space-y-3">
              {viewerData.medicalData.userName && (
                <div>
                  <p className="text-xs text-gray-500">Nombre</p>
                  <p className="text-sm text-gray-800 dark:text-gray-200">{viewerData.medicalData.userName}</p>
                </div>
              )}
              {viewerData.medicalData.bloodType && (
                <div>
                  <p className="text-xs text-gray-500">Tipo de Sangre</p>
                  <p className="text-sm text-gray-800 dark:text-gray-200">{viewerData.medicalData.bloodType}</p>
                </div>
              )}
              {viewerData.medicalData.allergies && (
                <div>
                  <p className="text-xs text-gray-500">Alergias</p>
                  <p className="text-sm text-gray-800 dark:text-gray-200">{viewerData.medicalData.allergies}</p>
                </div>
              )}
              {viewerData.medicalData.conditions && (
                <div>
                  <p className="text-xs text-gray-500">Condiciones</p>
                  <p className="text-sm text-gray-800 dark:text-gray-200">{viewerData.medicalData.conditions}</p>
                </div>
              )}
              {viewerData.medicalData.medications && (
                <div>
                  <p className="text-xs text-gray-500">Medicamentos</p>
                  <p className="text-sm text-gray-800 dark:text-gray-200">{viewerData.medicalData.medications}</p>
                </div>
              )}
      ...
      {isAdmin && (
        <div className="bg-white dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700/50 rounded-xl p-6 mb-6">
          <h2 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-4">Plan y Vencimiento</h2>
          <div className="flex flex-wrap items-center gap-4 mb-4">
            ...
            {tag.previousPlan && (
              <div>
                ...
                <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-amber-500/10 text-amber-600 dark:text-amber-400">
            ...
            {tag.expiresAt && (() => {
              ...
              return (
                <div>
                  ...
                  <p className={`text-sm font-medium ${diffDays > 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}`}>
          ...
          <div className="border-t border-gray-200 dark:border-gray-700/50 pt-4 mt-4">
            ...
            <div className="grid sm:grid-cols-3 gap-3">
              <button
                ...
                className="px-4 py-2.5 bg-gray-100 dark:bg-gray-700/50 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 text-sm font-medium rounded-lg transition-colors"
              >
                Principal — $99/año
              </button>
              <button
                onClick={() => handleRenew('PRO')}
                className="px-4 py-2.5 bg-amber-500/10 hover:bg-amber-500/20 text-amber-600 dark:text-amber-400 text-sm font-medium rounded-lg transition-colors"
              >
                Pro — $299/año
              </button>
              <button
                onClick={() => handleRenew('ULTRA')}
                className="px-4 py-2.5 bg-red-500/10 hover:bg-red-500/20 text-red-600 dark:text-red-400 text-sm font-medium rounded-lg transition-colors"
              >
                Ultra — $449/año
              </button>
            </div>
            <p className="text-xs text-gray-400 dark:text-gray-600 mt-2">El nuevo año corre desde la fecha de renovación. Si el tag estaba degradado, se restaura el plan original.</p>
          </div>
        </div>
      )}
      ...
      <div className="bg-white dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700/50 rounded-xl overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700/50">
          <h2 className="text-sm font-medium text-gray-500 dark:text-gray-400">Historial de Escaneos</h2>
        </div>
        {scans.length === 0 ? (
          ...
          <table className="w-full text-sm min-w-[560px]">
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700/30">
              {scans.map((scan: any) => (
                <tr key={scan.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/20">
                  <td className="px-6 py-3 text-gray-700 dark:text-gray-300 flex items-center gap-2">
                    <Calendar className="w-3.5 h-3.5 text-gray-500" />
                  ...
                  <td className="px-6 py-3 text-gray-500 dark:text-gray-400 flex items-center gap-2">
      ...
      {isAdmin && (
      <div className="bg-white dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700/50 rounded-xl overflow-hidden mt-6">
        <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700/50">
          ...
          <table className="w-full text-sm min-w-[640px]">
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700/30">
              {alerts.map((alert: any) => (
                <tr key={alert.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/20">
                  <td className="px-6 py-3 text-gray-700 dark:text-gray-300 flex items-center gap-2">
                    <Calendar className="w-3.5 h-3.5 text-gray-500" />
                  ...
                  <td className="px-6 py-3">
                    <div className="flex items-center gap-2">
                      <span className="text-green-600 dark:text-green-400 text-xs">{alert.successCount}✓</span>
                      {alert.failCount > 0 && (
                        <span className="text-red-600 dark:text-red-400 text-xs">{alert.failCount}✗</span>
                      )}
                    </div>
                  </td>
                  <td className="px-6 py-3 text-gray-500 dark:text-gray-400 flex items-center gap-2">
      ...
      >
        ...
        <div className="flex gap-3">
          <button
            ...
            className="flex-1 py-2.5 px-4 bg-gray-100 dark:bg-gray-700/50 text-gray-700 dark:text-gray-300 font-medium rounded-lg hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors"
          >
            Cancelar
          </button>
          <button
